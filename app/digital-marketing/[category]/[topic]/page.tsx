...
export async function generateMetadata({ params }: TopicPageProps): Promise<Metadata> {
  ...
  return {
    title: `${topic.title} – Digital Marketing Tutorial`,
    description: topic.description,
    keywords: [...topic.tags, category.name, "digital marketing", "marketing tutorial"],
    alternates: { canonical: `/digital-marketing/${category.slug}/${topic.slug}` },
    openGraph: {
      type: "article",
      title: `${topic.title} – Digital Marketing Tutorial | ${siteConfig.name}`,
      description: topic.description,
      url: `${siteConfig.url}/digital-marketing/${category.slug}/${topic.slug}`,
...
export default async function DigitalMarketingTopicPage({ params }: TopicPageProps) {
  const { category: categorySlug, topic: topicSlug } = await params;
  const section = getCurriculumSection(SECTION_SLUG);
  const category = getCurriculumCategory(SECTION_SLUG, categorySlug);
  const topic = getCurriculumTopic(SECTION_SLUG, categorySlug, topicSlug);
